...
export function PayslipForm({
  ...
  const methods = useForm<PayslipInput>({
  ...
  // Auto-fill the basic salary only when the user actually switches employee —
  // never on mount, which would overwrite a stored payslip's salary in edit mode.
  const employeeId = watch("employeeId");
  const prevEmployeeRef = useRef(employeeId);
  useEffect(() => {
    if (prevEmployeeRef.current === employeeId) return;
  ...
  function onSubmit(values: PayslipInput) {
    startTransition(async () => {
      const res =
        mode === "edit"
          ? await updatePayslipAction(payslipId!, values)
          : await createPayslipAction(values);
      if (res && !res.ok) {
        if (res.fieldErrors) {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" noValidate>
          ...
          <div className="flex items-center gap-3">
            <Button type="submit" disabled={pending} className="h-11">
              ...
              {mode === "edit" ? "Save changes" : "Generate payslip"}
            </Button>
            <Button asChild variant="ghost" className="h-11">
              <Link href={mode === "edit" && payslipId ? `/payslips/${payslipId}` : "/payslips"}>
                Cancel
              </Link>
            </Button>
          </div>
